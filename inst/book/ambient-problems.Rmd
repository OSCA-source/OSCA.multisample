...
This allows readers to make up their own minds as to whether a particular DEG's effect is driven by ambient contamination, in much the same way as described for low-quality cells in `r link("marking-qc", "OSCA.basic")`).
...
ctrl.ambient <- ambientContribControl(counts(summed.neural), ambient,
...
con.ambient.proxy <- ambientContribControl(counts(summed.neural), 
